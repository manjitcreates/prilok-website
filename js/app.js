        ...
        materialsGrid.innerHTML = filtered.map(item => {
            ...
            return `
                <div class="glass-card rounded-2xl p-5 flex flex-col justify-between space-y-3 border border-slate-800 hover:border-amber-500/50 transition-all group overflow-hidden hover:-translate-y-1">
                    ...
                    <div class="relative h-40 w-full rounded-xl overflow-hidden bg-navy-950 border border-slate-800">
                        ...
                        <div class="absolute top-2 right-2 z-10">
                            <span class="flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full backdrop-blur-sm ${item.inStock ? 'bg-emerald-950/90 text-emerald-400 border border-emerald-500/40' : 'bg-amber-950/90 text-amber-400 border border-amber-500/40'}">
                                <span class="w-1.5 h-1.5 rounded-full ${item.inStock ? 'bg-emerald-400' : 'bg-amber-400'} mr-1"></span>
                                ${item.inStock ? 'In Stock UAE' : 'Direct Import'}
                            </span>
                        </div>
                    </div>
